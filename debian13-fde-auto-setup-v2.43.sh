...
msg()  { echo -e "${BOLD}${BLUE}[*]${NC} $*" >&2; }
ok()   { echo -e "${BOLD}${GREEN}[✓]${NC} $*" >&2; }
warn() { echo -e "${BOLD}${YELLOW}[!]${NC} $*" >&2; }
...
select_install_mode() {
    echo -e "\n═════════════════════════════════════════════════════════\n" >&2
    echo -e "${CYAN}Debian 13 LUKS Full Disk Encryption Setup Script${NC}" >&2
    echo -e "${CYAN}Select Operation Mode:${NC}\n" >&2

    echo -e "  ${BOLD}${YELLOW}1) Fresh FDE desktop install${NC}" >&2
    echo -e "  ${DIM}   (Wipes everything)${NC}" >&2
    echo -e "     • Creates new partition table" >&2
    echo -e "     • Sets up fresh LUKS encryption" >&2
    echo -e "     • Installs new desktop environment\n" >&2

    echo -e "  ${BOLD}${YELLOW}2) Re-install FDE desktop${NC}" >&2
    echo -e "  ${DIM}   (Keep user data)${NC}" >&2
    echo -e "     • Keeps existing partition structure" >&2
    echo -e "     • Re-installs new desktop environment" >&2
    echo -e "     • Preserves all home data" >&2
    echo -e "     • Preserves LUKS keys" >&2
    echo -e "     • Formats only root and swap\n" >&2

    echo -e "  ${BOLD}${YELLOW}3) Change LUKS Keys${NC}" >&2
    echo -e "  ${DIM}   (Key management only)${NC}" >&2
    echo -e "     • Change encryption password" >&2
    echo -e "     • Regenerate keyfile for existing system" >&2
    echo -e "     • No system modifications\n" >&2

    echo -e "  ${BOLD}${YELLOW}<< BACKUP YOUR DATA BEFORE PROCEEDING!! >>${NC}\n" >&2
    echo -e "═════════════════════════════════════════════════════════\n" >&2
...
select_encryption_mode() {
    echo -e "\n${CYAN}Select Encryption Mode:${NC}\n" >&2

    echo -e "  ${BOLD}${YELLOW}1) Single Password Mode${NC}" >&2
    echo -e "     • Encrypted /boot partition on disk" >&2
    echo -e "     • Encrypted LVM partition on disk" >&2
    echo -e "     ${GREEN}✓${NC} Keyfile on /boot auto-unlocks LVM" >&2
    echo -e "     ${GREEN}✓${NC} Single password unlocks everything" >&2
    echo -e "     ${RED}✗${NC} Password always required" >&2
    echo -e "     ${RED}✗${NC} Slower boot time\n" >&2

    echo -e "  ${BOLD}${YELLOW}2) USB Keyfile Mode${NC}" >&2
    echo -e "     • Unencrypted EFI/BIOS + /boot on USB" >&2
    echo -e "     • Only encrypted LVM partition on disk" >&2
    echo -e "     ${GREEN}✓${NC} Keyfile on USB unlocks LVM" >&2
    echo -e "     ${GREEN}✓${NC} Automatic boot (no password needed)" >&2
    echo -e "     ${RED}✗${NC} USB stick always required" >&2
    echo -e "     ${RED}✗${NC} USB theft/loss compromises security\n" >&2
...
if [[ "$INSTALL_MODE" == "fresh" ]]; then
    ...
    echo -e "${CYAN}Found LUKS partitions:${NC}" >&2
    ...
    if [[ "$BOOT_PART_NUM" != "$LUKS_PART_NUM" ]]; then
        ...
        # Detect partitions on USB
        USB_BOOT_PART="${USB_DEVICE}1"
        USB_KEYFILE_PART="${USB_DEVICE}2"
        if [[ "$BOOT_MODE" == "UEFI" ]]; then
            USB_EFI_PART="${USB_DEVICE}1"
            USB_BOOT_PART="${USB_DEVICE}2"
            USB_KEYFILE_PART="${USB_DEVICE}3"
        fi
        msg "Detected USB keyfile mode (EFI/BIOS + boot on USB)"
    fi
    ...
    echo -e "\n${CYAN}Found LUKS partitions:${NC}" >&2
    echo "$LUKS_PARTS" | sed 's/^/  /' >&2
    echo >&2
    ...
    if [[ "$LUKS_PART_COUNT" -ge 2 ]]; then
        ...
        read -rp "Enter USB device for EFI/BIOS + boot + keyfile (e.g. /dev/sdb): " USB_DEVICE
        [[ -b "$USB_DEVICE" ]] || die "Invalid USB device: $USB_DEVICE"
        ...
        msg "Detected USB keyfile mode (EFI/BIOS + boot on USB)"
        msg "LUKS partition: $LUKS_PART"
        msg "USB boot partition: $USB_BOOT_PART"
...
if [[ "$INSTALL_MODE" != "change-keys" ]]; then
    ...
    echo -e "  ${DIM}(minimum disk space required)${NC}" >&2
    echo -e "  1) KDE Plasma  ${DIM}(10GB)${NC}" >&2
    echo -e "  2) GNOME       ${DIM}(15GB)${NC}" >&2
    echo -e "  3) MATE        ${DIM}(8GB)${NC}" >&2
    echo -e "  4) XFCE        ${DIM}(6GB)${NC}" >&2
    echo -e "  5) <Cancel>\n" >&2
...
rebuild_initramfs_and_grub() {
    ...
    # Open boot partition
    echo -e "${YELLOW}Enter boot partition password:${NC}" >&2
    ...
    # Rebuild initramfs (embeds new keyfile) and GRUB
    echo >&2
...
change_luks_keys() {
    ...
    if [[ "$ENCRYPTION_MODE" == "password" ]]; then
        echo -e "  ${BLUE}Password Mode Key Change Options:${NC}\n" >&2
        ...
        while true; do
            ...
            case "$key_choice" in
                ...
                2)
                    ...
                    echo -e "\n${YELLOW}Enter current LVM partition password:${NC}" >&2
                ...
                3)
                    ...
                    echo -e "\n${YELLOW}Enter boot partition password to open it:${NC}" >&2
                    ...
                    ;;
                4)
                    echo -e "\n${YELLOW}Step 1: Change boot partition password${NC}" >&2
                    cryptsetup luksChangeKey "$BOOT_PART"

                    echo -e "\n${YELLOW}Step 2: Open boot partition with new password${NC}" >&2
                    cryptsetup open "$BOOT_PART" crypt-boot-temp || die "Failed to open boot partition"
                    mkdir -p /mnt/boot-temp
                    mount /dev/mapper/crypt-boot-temp /mnt/boot-temp || {
                        cryptsetup close crypt-boot-temp
                    ...
                    echo -e "\n${YELLOW}Step 3: Replace LVM partition keyfile in keyslot ${OLD_KEYFILE_SLOT}${NC}" >&2
                    if ! cryptsetup luksChangeKey "$LUKS_PART" /mnt/boot-temp/luks-lvm.keyfile.new \
                            --key-file /mnt/boot-temp/luks-lvm.keyfile \
                            --key-slot "$OLD_KEYFILE_SLOT"; then
                        warn "luksChangeKey failed. No changes were committed."
                    ...
                    echo -e "\n${YELLOW}Step 4: Change LVM partition backup password${NC}" >&2
                    cryptsetup luksChangeKey "$LUKS_PART"
                    echo >&2
        ...
        # USB keyfile mode
        echo -e "  ${BLUE}USB Keyfile Mode Key Change Options:${NC}\n" >&2
        ...
        while true; do
            ...
            case "$key_choice" in
                ...
                3)
                    ...
                    echo -e "\n${YELLOW}Step 1: Replace keyfile in keyslot ${OLD_KEYFILE_SLOT} on ${LUKS_PART}${NC}" >&2
                    if ! cryptsetup luksChangeKey "$LUKS_PART" /mnt/usb-keyfile/luks-lvm.keyfile.new \
                            --key-file /mnt/usb-keyfile/luks-lvm.keyfile \
                            --key-slot "$OLD_KEYFILE_SLOT"; then
                        warn "luksChangeKey failed. No changes were committed."
                    ...
                    echo -e "\n${YELLOW}Step 2: Change LVM backup password${NC}" >&2
                    cryptsetup luksChangeKey "$LUKS_PART"
...
if [[ "$INSTALL_MODE" == "change-keys" ]]; then
    ...
    if [[ "$ENCRYPTION_MODE" == "password" ]]; then
        echo -e "\n ${GREEN}•${NC} ${BOLD}Initramfs and GRUB were rebuilt automatically.${NC}" >&2
        echo -e " ${GREEN}•${NC} Test the new keys by rebooting ${BOLD}before${NC} relying on this system." >&2
    else
        echo -e "\n ${GREEN}•${NC} USB keyfile mode: initramfs does not embed the keyfile." >&2
        echo -e " ${GREEN}•${NC} Test the new USB keyfile by rebooting ${BOLD}before${NC} relying on this system." >&2
    fi
    echo -e " ${GREEN}•${NC} ${YELLOW}Backup LUKS headers to a secure location (offline).${NC}" >&2
    echo -e " ${GREEN}•${NC} For troubleshooting you can find the log file at:" >&2
    echo -e "   /var/log/debian-fde-setup.log${NC}" >&2
    echo -e "\n═════════════════════════════════════════════════════════\n" >&2
...
partition_target_disk() {
    ...
    if [[ "$BOOT_MODE" == "UEFI" ]]; then
        if [[ "$mode" == "password" ]]; then
            ...
            # UEFI: only LUKS partition on disk (EFI + boot on USB)
            sgdisk -n 1:0:0 -t 1:8300 -c 1:"LUKS" "$dev"
            LUKS_PART="${dev}${sfx}1"
        fi
    else
        if [[ "$mode" == "password" ]]; then
            ...
            # BIOS: only LUKS partition on disk (BIOS + boot on USB)
            sgdisk -n 1:0:0 -t 1:8300 -c 1:"LUKS" "$dev"
            LUKS_PART="${dev}${sfx}1"
        fi
    fi
...
partition_usb_device() {
    local dev="$1"
    step "Partitioning USB device $dev (EFI/BIOS + boot + keyfile)"
    ...
    if [[ "$BOOT_MODE" == "UEFI" ]]; then
        # UEFI: EFI + BOOT + KEYFILE
        sgdisk -n 1:0:+${EFI_PART_SIZE} -t 1:EF00 -c 1:"EFI" "$dev"
        sgdisk -n 2:0:+${BOOT_PART_SIZE} -t 2:8300 -c 2:"BOOT" "$dev"
        sgdisk -n 3:0:0 -t 3:8300 -c 3:"KEYFILE" "$dev"
        USB_EFI_PART="${dev}${sfx}1"
        USB_BOOT_PART="${dev}${sfx}2"
        USB_KEYFILE_PART="${dev}${sfx}3"
    else
        # BIOS: BIOS + BOOT + KEYFILE
        sgdisk -n 1:0:+1M -t 1:EF02 -c 1:"BIOS" "$dev"
        sgdisk -n 2:0:+${BOOT_PART_SIZE} -t 2:8300 -c 2:"BOOT" "$dev"
        sgdisk -n 3:0:0 -t 3:8300 -c 3:"KEYFILE" "$dev"
        USB_BOOT_PART="${dev}${sfx}2"
...
setup_encryption() {
    ...
    if [[ "$INSTALL_MODE" == "fresh" ]]; then
        if [[ "$ENCRYPTION_MODE" == "password" ]]; then
            ...
            # USB keyfile mode - EFI/BIOS + boot on USB, only LUKS on disk
            msg "Formatting USB partitions..."
            if [[ "$BOOT_MODE" == "UEFI" ]]; then
                mkfs.fat -F 32 -n EFI "$USB_EFI_PART" >/dev/null
            fi
        ...
        if [[ "$ENCRYPTION_MODE" == "password" ]]; then
            ...
            echo >&2
            echo -e "${YELLOW}Enter password to unlock boot partition:${NC}" >&2
            cryptsetup open "$BOOT_PART" crypt-boot || die "Failed to open boot partition"
...
if [[ "$ENCRYPTION_MODE" == "password" ]]; then
    ...
    # USB keyfile mode - mount USB boot partition
    mount "$USB_BOOT_PART" /mnt/luks-root/boot
    if [[ "$BOOT_MODE" == "UEFI" ]]; then
        mkdir -p /mnt/luks-root/boot/efi
        mount "$USB_EFI_PART" /mnt/luks-root/boot/efi
    fi
fi
...
if [[ "$ENCRYPTION_MODE" == "usb-keyfile" ]]; then
    ...
    if [[ "$BOOT_MODE" == "UEFI" ]]; then
        : "${USB_EFI_PART:?USB_EFI_PART not set}"
    fi
fi

if [[ "$BOOT_MODE" == "UEFI" && "$ENCRYPTION_MODE" == "password" ]]; then
...
if [[ "$ENCRYPTION_MODE" == "password" ]]; then
    ...
    # USB boot partition - mounted by initramfs
...
if [[ "$BOOT_MODE" == "UEFI" ]]; then
    if [[ "$ENCRYPTION_MODE" == "password" ]]; then
        ...
        echo "UUID=$(blkid -s UUID -o value "$USB_EFI_PART")  /boot/efi  vfat  defaults,noauto,nofail  0 1" >> /etc/fstab
    fi
fi
ok "fstab configured"
...
EOF
    ...
    # USB keyfile mode - only crypt-disk on internal disk
    cat > /etc/crypttab << EOF
...
# Create initramfs hook for keyfile permissions
...
if [ "$ENCRYPTION_MODE" = "usb-keyfile" ]; then
    ...
    if [ -n "$USB_UUID" ]; then
        ...
        if [ "$BOOT_MODE" = "UEFI" ]; then
            mkdir -p /boot/efi
            EFI_UUID=$(blkid -s UUID -o value "$USB_EFI_PART" 2>/dev/null)
            if [ -n "$EFI_UUID" ]; then
...
if [[ "$ENCRYPTION_MODE" == "password" ]]; then
    ...
    # USB keyfile mode - install GRUB to USB device
    if [[ "$BOOT_MODE" == "UEFI" ]]; then
...
chroot /mnt/luks-root /bin/bash -c "
    ...
    export USB_EFI_PART='${USB_EFI_PART:-}'
...
if [[ "$INSTALL_MODE" == "reinstall" ]]; then
    echo -e "  ${DIM}(Note: You can reuse the previous username to pick up all existing configs and settings without the extra restoration work)${NC}\n" >&2
fi
...
while true; do
    read -rsp "Enter password for '${new_user}': " pass1
    echo >&2
...
echo -e "\n${YELLOW}----------------------------------------------${NC}" >&2
echo -e "${YELLOW}${BOLD}|  ⚠️  IMPORTANT:                             |${NC}" >&2
echo -e "${YELLOW}|  Root password is 'changeme' - CHANGE IT!  |${NC}" >&2
echo -e "${YELLOW}----------------------------------------------${NC}" >&2
...
if [[ "$INSTALL_MODE" == "fresh" ]]; then
    ...
    if [[ "$ENCRYPTION_MODE" == "password" ]]; then
        echo -e "\n${YELLOW}🔑 Remember: The password is the critical security factor so ${NC}" >&2
        echo -e "${YELLOW}   use a strong password${NC}" >&2
        echo -e "${RED}   Security will be fully compromised if exposed!${NC}\n" >&2

        echo -e " ${GREEN}•${NC} Backup password for LVM has been added (keyslot 1)" >&2
    else
        echo -e "\n${YELLOW}🔑 Remember: The USB stick is the 'keys to the castle'${NC}" >&2
        echo -e "${YELLOW}   Protect it just like physical keys${NC}" >&2
        echo -e "${RED}   Security will be fully compromised if discovered!${NC}\n" >&2

        echo -e " ${GREEN}•${NC} EFI/BIOS and /boot are on the USB stick (unencrypted)" >&2
        echo -e " ${GREEN}•${NC} Only the encrypted LVM partition is on the internal disk" >&2
        echo -e " ${GREEN}•${NC} Don't be lazy, ${CYAN}${BOLD}remove USB stick${NC} and store securely when not in use" >&2
        echo -e "\n ${GREEN}•${NC} Backup the USB to secure locations" >&2
        echo -e "   ${DIM}(use multiple backups, not just one)${NC}\n" >&2
        echo -e " ${GREEN}•${NC} Backup password for LVM has been added (keyslot 1) for recovery" >&2
    fi
else
    step "LUKS Desktop Re-install Summary:"
    echo -e "\n ${GREEN}•${NC} Partition structure, Home directory data, and LUKS encryption keys ${BLUE}preserved${NC}" >&2
    echo -e " ${GREEN}•${NC} New desktop environment installed: ${BLUE}$DE${NC}" >&2
fi

echo -e "\n ${GREEN}•${NC} For troubleshooting you can find the log file at: /var/log/debian-fde-setup.log${NC}" >&2
echo -e "\n ${GREEN}•${NC} After every upgrade, update the initramfs and grub before rebooting" >&2
echo -e "   ${DIM}(kernel updates could lock you out of your system!)${NC}\n" >&2
