...
Designed for the average user with some Linux experience who doesn't want the hassle of manually creating a LUKS FDE system in the terminal.
...
Carefully assess whether the security of this setup works for you. Secure boot and TPM have their own issues (eg. can you trust the hardware?). Plus FDE only works when the device is at rest: there is no protection while the device is powered on and running. See here for a deeper FDE analysis:
