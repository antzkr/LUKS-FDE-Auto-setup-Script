...
Consider adding a swapfile on first login, since a swap partition adds increasing complexity for no real significant advantage. Plus the size of a swapfile can be easily increased without restructuring the LVM layout. Strongly suggested for systems with less than 16GB available RAM.
