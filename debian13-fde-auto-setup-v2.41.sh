...
select_encryption_mode() {
    ...
    echo -e "  ${DIM}   (Very strong security)${NC}" >&2
    ...
    echo -e "     ${RED}✗${NC} Password always required at boot" >&2
    echo -e "     ${RED}✗${NC} Slower boot time\n" >&2

    echo -e "  ${BOLD}${YELLOW}2) USB Keyfile Mode${NC}" >&2
    echo -e "  ${DIM}   (Strong security)${NC}" >&2
    ...
    echo -e "     ${RED}✗${NC} USB stick required" >&2
...
if [[ "$INSTALL_MODE" == "fresh" ]]; then
    ...
    # Can rewrite the display of found luks partitions another way
    #printf '  %s\n' "${LUKS_PARTS//$'\n'/$'\n'  }" >&2
    echo >&2
...
while true; do
    ...
    if [[ "${pass1}" == "${pass2}" ]]; then
        ...
        #chroot /mnt/luks-root /bin/bash -c 'adduser "$1" --disabled-password --gecos "" && echo "$1:$2" | chpasswd && usermod -aG sudo "$1"' _ "${new_user}" "${pass1}"
